...
public sealed class NodeExecTool : ITool
{
    ...
    public string Name => "node_exec";

    public string Description => "Execute Node.js code (allowlisted, sandboxed working directory). Params: code (required), args (optional), working_dir (optional), timeout_ms (optional), max_output_bytes (optional).";
    ...
    public async Task<ToolResult> ExecuteAsync(Dictionary<string, object> parameters, CancellationToken ct = default)
    {
        ...
        if (!_options.EnableNode)
        {
            return Fail("Node.js execution is disabled (CodeExecution:EnableNode=false)");
        }
        ...
        _logger.LogInformation(
            "🟩 node_exec exit={ExitCode} timeout={TimedOut} truncated={Truncated} duration_ms={DurationMs}",
            result.ExitCode,
            result.TimedOut,
            result.Truncated,
