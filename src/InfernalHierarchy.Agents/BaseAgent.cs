...
public abstract class BaseAgent : IAgent
{
    ...
    {
        var transport = task.Payload != null && task.Payload.TryGetValue("transport", out var transportObj)
            ? transportObj?.ToString()
            : null;

        if (string.IsNullOrWhiteSpace(transport) && task.Payload?.ContainsKey("telegram_chat_id") == true)
        {
            transport = "telegram";
        }

        if (string.IsNullOrWhiteSpace(transport) && task.Payload?.ContainsKey("http_request_id") == true)
        {
            transport = "http";
        }
        ...
        context += $"""
            ...
            - The "From" field above is an internal reply channel id, not a user/chat identifier.
            - Do NOT use the send_telegram tool to reply to this task; your report will be delivered automatically.
            - Use send_telegram only when explicitly asked to message a different chat, or for out-of-band notifications.
            - If the user explicitly requests a tool (e.g., "use web_search"), attempt it at least once before declaring it broken.
            - Provide your response as a final report (FINAL_ANSWER).
